...
**验证强度**：`tsc` 0 错 · **273 个单元测试** · **29 个冒烟场景 / 608 条断言**
...
tests/                    273 个单元测试（node:test，纯函数优先）
scripts/
...
### 5.17 「先 build 再 smoke」是硬规矩（第二次发作，而且导致了错误诊断）
...
硬规矩：冒烟结果异常时，顺序是
① `node scripts/build.mjs` → ② 再跑一次复现 → ③ 才去读代码找原因。
在"没重建"这个前提被排除之前，任何从冒烟输出里得出的结论都不可信。
...
| 冒烟测试 | `scripts/smoke.mjs` | **端到端**：加载真实打包产物 + 假 Obsidian，29 个场景 / 608 条断言 |
