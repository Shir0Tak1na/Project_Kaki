...
- **图层开关**：隐藏某层后画布确实不画它、而地图文件里的数据仍在（"图层不改数据"这条承诺的验收；
  冒烟有断言，但真实布局下未人工确认）。
- **工具条「名称」按钮与设置里的开关是否始终一致**（"唯一真相"那条改动的验收点；
  冒烟里有回归断言盯着它，但真实界面下的表现未人工确认）。
- **自定义地形的图片**：真实库里 `app://` 资源地址能否被 `drawImage` 接受、
  SVG 图片尺寸为 0 的边角情况、大图解码耗时 —— 这三项在真实库上都没试过。
