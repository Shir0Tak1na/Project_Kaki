...
### 7. 修缺陷轮（2026-09-25 · 用户实测报的）

**用户报的现象**："设置中的插件设置页内，导入新定义的 UI 工作不正常"。
实际是：点「导入定义文件…」之后，文件选择器里**一个候选都没有** ——
连刚刚导出的那份定义文件也看不到，导入流程根本走不下去。

**根因**：图片选择器与定义文件导入**共用**的那个弹窗（`AssetSuggestModal`），
它的 `getItems()` **写死了图片扩展名白名单**，于是 `.json` 候选被全部筛掉 → 恒为空清单。
（详细根因、为什么测试全绿、以及这次的判据，写在 `ENGINEERING-NOTES.md` §5.30。）

**本轮改动**：

- `src/ui/AssetSuggestModal.ts`：新增 `kind: 'image' | 'bundle'`（**必填**，由调用方给），
  `getItems()` 按它选 `listImagePaths` / `listBundlePaths`；占位文案也按类别给
  （`选择库内图片…` / `选择定义文件…`）。
- `src/main.ts`：两个调用点各自显式声明类别。`kind` 是必填 ⇒ 漏传**编译期就过不去**（TS2345）。
- `scripts/smoke.mjs`：**+5 条断言（977 → 982）** —— 真实弹窗 `kind: 'bundle'` 的两条，
  以及**整条导入链路走真实选择器**的三条（清单非空且全是 `.json`、能看到刚导出的那份、选中后仍走到确认对话框）。
- 四道闸（在最终状态上实测）：构建 **59 模块 / 860.9 KiB** · `tsc` **0 错** · 单测 **404** · 冒烟 **982 条 0 失败**。
- 鉴别力实测：把 `getItems()` 改回写死图片白名单 → **3 条新断言立刻变红**（并如实报出 `["Assets/forest.png"]` 与 `[]`）；
  删掉调用点的 `kind` → **编译期直接失败**，产物新鲜度门禁紧接着拒绝用旧产物跑冒烟。
