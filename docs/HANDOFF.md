...
1. **文档轮只做了一半（本轮最要紧的欠账）**：`docs/ENGINEERING-NOTES.md` 已经更新到新基线并新增
   **§5.27（把类型联合收窄成内置集合，让查表在编译期过不去）/ §5.28（假 DOM 不模拟事件传播 ⇒ 断言空转）/
   §5.29（守卫今天不可观测时要写下来）**；`README.md` 与 `docs/HANDOFF.md` 的基线数字也已同步。
   **但 `README.md` 的功能列表、`docs/USER-MANUAL.md`、`CHANGELOG.md` 还没有写 ⑤（路径类型 / 区域类型）
   与 ③（定义文件四类导入导出）**：手册里目前只有「自定义地形」「自定义标记图标」两节，
   CHANGELOG 的 `[未发布]` 段也停在 ④。**下一轮第一件事就是把这三份文件补完**，
   并在 README 里说明「定义文件打包四类」而不是三类。
