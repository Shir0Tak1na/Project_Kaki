...
- 文档中的测试数量必须和实际输出同步。当前基线是 `273 / 608`（单元测试 / 冒烟断言，29 个冒烟场景），
  两者都能自己数出来：`node --test --test-isolation=none` 的**汇总行**（`ℹ tests 273`）、
  `node scripts/smoke.mjs` 的末行。
  ⚠️ **去读汇总行，不要目测**：这条基线曾被写错成 278，原因是用 dot reporter 的点数"数行数"。
- **冒烟结果异常时，第一件事是 `node scripts/build.mjs` 再复现，然后才看代码**：
  冒烟加载的是打包产物 `main.js`，改了源码不重新构建就会拿旧产物跑测试。
  这条陷阱已经发作两次，第二次还导致了一个**错误的诊断**（把"没重建"误判成"异常被 `void` 吞掉"）。
