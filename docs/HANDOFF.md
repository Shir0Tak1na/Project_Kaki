...
4. ✅ **已完成：PNG 导出**（复用 SVG 几何投影，命令与 SVG 导出共用命名逻辑）
   - **仍待在真实浏览器里点一次**：`canvas.toBlob` 与 `Image` 解码 data URL 的成功路径只经过注入替身，
     真实环境尚未人工验证（见 `ENGINEERING-NOTES.md` §7）。
...
- 文档中的测试数量必须和实际输出同步。当前基线是 `236 / 532`（单元测试 / 冒烟断言，26 个冒烟场景），
