# Project Kaki

**在 Obsidian 里画一张虚构世界的地图。**

[![CI](https://github.com/Shir0Tak1na/Project_Kaki/actions/workflows/ci.yml/badge.svg)](https://github.com/Shir0Tak1na/Project_Kaki/actions/workflows/ci.yml)
[![License](https://img.shields.io/badge/license-Apache--2.0-blue)](LICENSE)
![Obsidian](https://img.shields.io/badge/Obsidian-1.5.0%2B-7c3aed)
![Base 视图](https://img.shields.io/badge/Base%20view-1.10.0%2B-0f766e)

Project Kaki（译名 **Project 垣**）是一个 Obsidian 插件：把 `.canvas` 变成一张**可绘制的六边形地图**，
再用 `.base` 把地图和你的世界观笔记连起来 —— 地形、河流、国境、城市都在画布上画，
每个地标又能直接跳到对应的笔记。

> **English** — Project Kaki is an Obsidian plugin for drawing fictional world maps on hex grids
> inside `.canvas` files, and navigating them alongside your worldbuilding notes through `.base`.
> Terrain brushes, rivers/roads/borders, regions, markers and labels; a sidebar panel for common
> actions; SVG export. Requires Obsidian 1.5.0+ (the Base view needs 1.10.0+).
> Docs are in Chinese; see [`docs/USER-MANUAL.md`](docs/USER-MANUAL.md).

![示例地图](docs/images/preview.svg)

*上图的示例世界由插件自己的 SVG 导出渲染器生成（`node scripts/make-preview.mjs` 可重现）——
不是手绘示意图，跑的就是你导出时会走的那条代码路径。*

## 目录

- [特性](#特性) · [安装](#安装) · [快速上手](#快速上手) · [使用说明](#使用说明)
- [自定义地形](#自定义地形) · [自定义标记图标](#自定义标记图标) · [自定义路径类型](#自定义路径类型) · [自定义区域类型](#自定义区域类型)
- [定义文件](#定义文件把自定义定义带到别的库) · [Base 视图](#base-视图把地图和笔记连起来) · [导出](#导出) · [设置](#设置)
- [数据与文件](#数据与文件) · [兼容性与已知限制](#兼容性与已知限制) · [常见问题](#常见问题)
- [开发](#开发) · [路线图](#路线图) · [更新日志](CHANGELOG.md) · [许可证](#许可证) · [签名](#签名)

## 特性

| 能力 | 说明 |
|---|---|
| **六边形地形** | 9 种内置地形（平原 / 森林 / 丘陵 / 山脉 / 水域 / 沼泽 / 沙漠 / 冻原 / 火山），笔刷半径可调，拖动即落笔 |
| **路径** | 河流、道路、贸易路线、边界。河流末端**渐细**，名称**逐字贴着线条**排布 |
| **区域** | 半透明领域范围，名称按面积质心摆放（凹形状也会落在形状内部） |
| **标记与文字** | 地标可绑定笔记链接；文字标注随缩放保持可读字号 |
| **三种几何模式** | 路径与区域可选：**穿内部**（穿格心）/ **沿格边**（勾六边形边框）/ **逐边**（一次点一条边），见下 |
| **撤销 / 重做** | 一次拖动 = 一步历史，可整条笔画撤销、重做 |
| **地图面板** | 右侧边栏视图：常用动作一键触发，不用每次翻命令面板；开发用探针默认隐藏 |
| **可自定义样式** | 路径类型与区域类型各自的参数、名称字体族（可跟随主题）；「恢复出厂样式」一键还原（自定义类型的定义不会被删） |
| **自定义地形** | 自己定义地形类型（ID + 显示名 + 颜色 + 可选字形/图片）；改显示名不影响已画的格子，删定义也不会删格子 |
| **自定义标记图标** | 自己定义标记图标（ID + 显示名 + 字形或**库内图片**两种模式）；改显示名不影响已放置的标记，删定义也不会删标记 |
| **自定义路径类型** | 内置 4 种之外自己加类型，每种可调颜色 / 线宽 / 虚线 / 端点 / 连接；改显示名不影响已画的路径，删定义也不会删路径，**认不出的类型原样保留并照常绘制** |
| **自定义区域类型** | 内置 6 种之外自己加类型，每种可调填充色 / 不透明度 / 边框色 / 边框宽 / 边框虚线；旧地图不会被补字段，**认不出的类型原样保留并照常绘制** |
| **定义文件导入 / 导出** | 把四类自定义定义打包成一份 JSON（备份、换机器、分享给别人）；导入前先告诉你会新增几条、跳过几条，**只做补充，绝不删你现有的定义** |
| **图层开关** | 地形 / 网格 / 区域 / 路径 / 标记 / 名称 六层分别显示或隐藏（只影响显示，不改数据） |
| **图例** | 从地图上**实际有的内容**生成，色块体现路径虚线样式；可随时隐藏 |
| **Base 视图** | 表格 + 地图缩略图 + 笔记坐标桥：笔记只要写了坐标，就会出现在地图与缩略图上 |
| **SVG / PNG 导出** | 导出独立 SVG 或 1600×1000 位图，可选**范围**（全部内容 / 当前视口 / 某个区域）；与画布共用同一套世界坐标与配色 |

## 安装

插件只有三个文件：`main.js`、`manifest.json`、`styles.css`。任选一种方式装进
`<你的库>/.obsidian/plugins/project-kaki/`，然后在 Obsidian 里：设置 → 社区插件 → 打开「已安装插件」→ 启用 **Project Kaki**。

### 方式一：从 Release 下载（推荐）

到 [Releases](https://github.com/Shir0Tak1na/Project_Kaki/releases) 页面，在最新版本的 **Assets** 里
下载那三个文件，放进上面的目录。不需要 Node.js，也不需要构建。

### 方式二：用 BRAT 安装

装好 [BRAT](https://github.com/TfTHacker/obsidian42-brat) 后，运行命令
**BRAT: Add a beta plugin for testing**，填 `Shir0Tak1na/Project_Kaki`，回车即可（BRAT 会自动取 Release 里的文件）。

### 方式三：从源码构建（开发版）

需要 Node.js 18+。

```bash
git clone https://github.com/Shir0Tak1na/Project_Kaki.git
cd Project_Kaki
npm ci --ignore-scripts     # 只装开发依赖（typescript / obsidian 类型 / yaml / esbuild），没有生命周期脚本
node scripts/build.mjs      # 产出 main.js
```

把 `main.js`、`manifest.json`、`styles.css` **这三个文件**复制到你的库：

```
<你的库>/.obsidian/plugins/project-kaki/
```

> 只复制这三个文件，**不要**复制 `package.json`（它带 `"type": "module"`，会让 Obsidian 的
> CommonJS 加载混乱）。

## 快速上手

1. 打开（或新建）一个 `.canvas` 文件。
2. `Ctrl/Cmd+P` → **创建地图并绑定到当前 Canvas** → 输入地图名。
   会生成 `Maps/<名字>.map.md`（frontmatter + 一段 JSON 代码块），并自动打开。
3. `Ctrl/Cmd+P` → **启用/停用当前 Canvas 的地图层**（或点左侧边栏的地图图标打开「地图面板」再点一下）。
   画布上出现六边形网格与地形。
4. 按 `D` 进入绘制模式 → `1`–`9` 选地形 → 按住左键拖动。
5. 想把它和笔记连起来：`Ctrl/Cmd+P` → **创建地图 Base 文件（表格视图）** → 打开生成的 `.base` →
   把视图类型切到 **地图**。

## 使用说明

### 快捷键

| 键 | 作用 |
|---|---|
| `D` | 绘制模式 ⇄ 选择模式 |
| `B` / `M` / `T` / `P` / `R` | 工具：地形笔刷 / 标记 / 文字 / 路径 / 区域 |
| `1`–`9` | 切换地形类型 |
| `[` / `]` | 笔刷半径 −1 / +1 |
| `Enter` | 结束正在绘制的路径或区域（比双击更可靠） |
| `Esc` | 取消当前草稿；再按一次退回选择模式 |
| `Ctrl/Cmd+Z` / `Ctrl/Cmd+Shift+Z` | 撤销 / 重做（**仅在绘制模式下接管**；选择模式下交还 Obsidian 的编辑器撤销） |

鼠标：左键绘制或拖动实体；**中键平移**与**滚轮缩放**始终是 Obsidian 原生行为；
双击路径/区域可**重命名**；**右键**删除形状（空白处右键仍弹原生菜单）。

### 三种几何模式

画路径或区域时，工具条上可以切换三种模式 —— 这是为了"看上去更整洁"：

| 模式 | 行为 | 适合 |
|---|---|---|
| **穿内部** | 直线连点，穿过格子内部（默认） | 河流、自然边界等有机形状 |
| **沿格边** | 自动沿六边形边走，勾勒出格边轮廓 | 国境线、行政区、整齐的领地 |
| **逐边** | 每次点击只前进**一条边**，方向由你点的位置决定 | 精细控制每一段走向的工整线条 |

切到沿格边/逐边后，**平滑会被关掉**（平滑会把格边抹成曲线，那就白画了）。
沿格边的路径会比直线距离长约 2 倍（三条边方向相隔 120°，需要"绕"）。

### 自定义地形

设置 → Project Kaki → 「自定义地形」里可以加自己的地形。每条先选**模式**，再填该模式需要的字段：

| 模式 | 用什么画 | 需要填 |
|---|---|---|
| **调色** | 颜色 + 字形（可借用某个内置地形的字形） | 显示名、颜色、字形。**不依赖任何外部资源**，最不容易失败，也是新建时的默认模式 |
| **图片** | 库内的一张图片当图块 | 显示名、图片、以及**回退色** —— 图片缺失或解不开时用它 + 字形顶上 |

图片模式还有一项**显示方式**：

| 显示方式 | 效果 |
|---|---|
| **单格一张**（默认） | 每个六边形各显示一张完整图片 |
| **整片一张（连通区域）** | 所有**连通的**同类型格共用一张图：图片按"完整装下"缩放（**保持比例**，不会把圆形湖泊拉成椭圆），并**只画在连通区域内** —— 超出部分被裁掉，不会盖到旁边别的地形上 |

> "连通"按**六边形的邻接**判定：中间隔着别的类型（例如一条海）的两片地算两块，各自一张图。
> 旧设置会自动推断为"单格一张"，升级后画面与之前一致。

每条定义还有两个公共字段：

| 字段 | 说明 |
|---|---|
| **ID** | **就是写进地图文件的标识**。只能小写字母开头、`a-z 0-9 _ -`，2–32 字符；`custom:` 前缀由插件自动补上（你自己带也会被去掉再补，不会出现两个前缀）。大写会被统一成小写 —— 否则 `Swamp` 与 `swamp` 会变成两条看起来一样、数据却互不相认的地形 |
| **显示名** | 只出现在工具条、图例与设置页。**改它不会影响任何已画好的格子**（文件里存的是 ID） |

几条需要知道的行为：

- **切换模式不会丢另一边的配置**：图片路径与字形都保留着，切回来还在。所以你可以放心地来回试，
  选图片模式时也不必担心原来的字形被清掉。只有在设置页**删除某条定义**才会真的丢。
- **图片那一栏在两种模式下都显示**（藏起来就等于没有这个功能）：在「调色」模式下点「从库中选择…」
  会**自动切到「图片」模式**再让你挑图；直接填一个合法路径也一样会自动切过去。
- **图片只能从库里选**（或手打库内相对路径）：Obsidian 没给插件开系统文件对话框，
  所以流程是"先把图片放进库 → 再选它"。好处是地图文件里存的是库内相对路径，
  换机器或同步之后仍然找得到。选择器只列**库内**的图片；库里没有图片时会明确提示，而不是弹一个空列表。
- 升级兼容：旧设置（只有"有图片就画图片"的隐含规则）会**自动推断成对应模式**，升级后画面与升级前一致。
- 自定义地形出现在工具条上**内置 9 种之后**；它们**不占用数字键** `1`–`9`（键位已被内置占满，再抢会破坏肌肉记忆），用鼠标点选即可，悬停提示里有完整 ID。
- **图片只在画布上生效**：Base 缩略图与 SVG 导出里的自定义地形使用回退色（导出文件要能脱离你的库打开，内联图片属于后续工作）。
- 图片**不存在 / 解不开 / 路径不合法**时，那一格回退成"颜色 + 字形"，并在控制台给出一条可读原因 —— 不会静默变成空白。
- **删掉设置里的定义不会删掉地图上的格子**：数据原样保留，只是视觉回退成"未知地形"的占位样式（这样你还能看到"这里有东西"，而不是地图上出现一个洞）。
- 最多 40 条：图集是一行位图（每条占一列 260 px），再多会超过部分平台的画布宽度上限，届时地形会静默不显示。

### 自定义标记图标

内置 9 种标记图标（城市 / 村镇 / 要塞 / 遗迹 / 港口 / 神殿 / 山峰 / 洞穴 / 塔）不够用时，
可以在设置 → Project Kaki → 「自定义标记」里加自己的图标。每条先选**模式**，再填该模式需要的字段：

| 模式 | 用什么画 | 需要填 |
|---|---|---|
| **字形** | 借用某个内置图标的形状（下拉里选，另有「通用」= 一个圆点）。**不依赖任何外部资源**，新建时默认 | 显示名、字形 |
| **图片** | 库内的一张图片 | 显示名、图片 |

每条定义的两个公共字段与自定义地形完全一致：

| 字段 | 说明 |
|---|---|
| **ID** | **就是写进地图文件的标识**（例如 `custom:lighthouse`）。小写字母开头、`a-z 0-9 _ -`、2–32 字符，`custom:` 前缀由插件自动补上；显示名与它解耦，**改显示名不影响已经放好的标记** |
| **显示名** | 只出现在工具条按钮、设置页与放置对话框的图标下拉里 |

几条需要知道的行为：

- **图片那一栏在两种模式下都显示**：在「字形」模式下点「从库中选择…」会**自动切到「图片」模式**再让你挑；
  直接填一个合法路径也一样会自动切过去（填图片路径的意图是明确的）。
- **图片只能从库里选**（或手打库内相对路径），与自定义地形共用同一套校验：
  只接受 `png / jpg / jpeg / webp / svg / gif` 的库内路径，选择器也只列这些。
- **设置页会直接把图画出来**做预览（按原比例缩放，**不拉伸**）；图片取不到时那里会写明原因
  （"当前取不到这张图的资源地址……画布上会退回字形"），而不是显示一个破图。
- **图片打不开时回退成这条标记的「字形」**（没配字形就是「通用」圆点），并在控制台给出一条可读原因 ——
  标记**不会**因为图挂了就消失；同一张坏图只尝试一次、只告警一次（否则会变成每帧一次 404 + 每帧一条日志）。
- 自定义标记出现在工具条上**内置 9 种之后**（绘制模式 + 标记工具时才显示这一组），按钮上带**显示名**
  （字形可能只是通用圆点，光看图分不出哪个是哪个），悬停提示里有完整 ID；
  **图片模式下按钮上直接显示你那张图**，而不是借来的字形。
- 放置标记对话框（`M`）的「图标」下拉里同样列出它们（下拉里是显示名，写进文件的是 ID）。
- **未知图标不会被改写**：文件里写了本机没有定义的图标名时，插件照常把它画出来（回退图钉）、
  把文件里的值**原样保留**，并在状态报告里计入「告警 N 条」、在控制台写清是哪个图标被保留了。
  （旧版本会把它改写成 `town`，保存一次就永久改了你的数据 —— 这是个已修的缺陷。）
- **删掉设置里的定义不会删掉地图上的标记**：数据原样保留（文件里的 ID 不变），只是视觉回退成回退图钉；
  加回一条同 ID 的定义就能恢复原样。最多 60 条。
- **标记图标只在画布上生效**：SVG / PNG 导出与 Base 缩略图里的标记一律是一个固定的小圆点
  （导出文件要能脱离你的库打开，内联图片属于后续工作）。

### 自定义路径类型

内置 4 种路径类型（河流 / 道路 / 贸易路线 / 边界）不够用时，
可以在设置 → Project Kaki → 「路径类型」里加自己的类型。

| 项 | 说明 |
|---|---|
| **ID** | **就是写进地图文件的值**（例如 `custom:trail`）。规则与自定义地形、自定义标记完全一致：小写字母开头、`a-z 0-9 _ -`、2–32 字符，`custom:` 前缀由插件自动补上 |
| **显示名** | 只出现在工具条下拉、设置页与图例里。**改它不影响任何已经画好的路径**（文件里存的是 ID；留空就用 ID） |
| **颜色 / 线宽 / 虚线 / 端点 / 连接** | 这条类型自己的画法。线宽是世界单位（1–40）；虚线留空 = 实线；端点与连接的缺省都是「圆头 / 圆角」（= 升级前的行为，旧地图逐像素不变） |
| **上限** | 32 条自定义路径类型 |

几条需要知道的行为：

- 路径工具下有**一个类型下拉**（内置 4 种在前、你的自定义类型在后），里面就是当前设置里存在的所有类型；
  内置 4 种的**名字固定**（不能改），参数可以改；
- **认不出的类型原样保留，而且照常画出来**：地图文件里写了本机没有定义的 `type`
  （别的库写的、别的版本写的、或者你自己手工填的），插件会照常绘制它（按回退样式）、
  **把文件里的值一个字符都不改**，并在加载时给出一条「未知路径类型 …，已保留」的告警。
  下拉的触发按钮会显示「未知（ID）」，而不会悄悄跳回第一个类型；
- **删掉设置里的定义不会删掉地图上的路径**：数据原样保留，只是画成回退样式；
- **改某条类型的参数只影响以后新画的对象**：已画好的路径把颜色、线宽这些值存在地图文件里
  （`path.color` / `path.width`…），所以改设置不会悄悄改掉你已经画好的地图；
- 「恢复出厂样式」只把**内置 4 种**路径类型与 **6 种**区域类型的参数、以及字体恢复为出厂默认，
  **不会删掉任何自定义类型定义**。

### 自定义区域类型

区域也是一个可扩展的类型目录，内置 6 种（王国 / 帝国 / 公国 / 教区 / 荒原 / 海域）；
在设置 → Project Kaki → 「区域类型」里可以加自己的类型。

| 项 | 说明 |
|---|---|
| **ID** | 与自定义地形、自定义标记、自定义路径类型同一条口径：小写字母开头、`a-z 0-9 _ -`、2–32 字符，`custom:` 前缀由插件自动补上。**它才是写进地图文件的值**，显示名只是界面文案 |
| **显示名** | 出现在工具条下拉、设置页与图例里（留空就用 ID） |
| **填充色 / 不透明度** | 这条类型自己的填充与透明度（不透明度 0–1，夹在范围内） |
| **边框色 / 边框宽 / 边框虚线** | 边框宽是世界单位（0–40，**0 = 不画边框**）；边框虚线留空 = 实线；边框色留空 = **跟随填充色**（这就是升级前的行为） |
| **上限** | 32 条自定义区域类型 |

几条需要知道的行为：

- 区域工具下同样是**一个类型下拉**（内置 6 种在前、你的自定义类型在后），每个选项带**自己的色块**，
  所以两条颜色相近的类型也分得清（否则界面上会出现两个看起来一样的选项）；悬停提示里有 ID 与画法参数；
- **旧地图不会被补字段**：升级前画的区域没有 `type`，插件读取时保持"没有"、写回时也**不补** ——
  老地图文件逐字节不变，图例仍然按颜色显示原来的类型名字；
- 未知区域类型与路径类型同一条承诺：**原样保留 + 告警 + 照常绘制**，
  **"类型不认识"绝不导致区域被丢弃**；
- 类型参数同样**只影响新画的对象**：区域把 `color` / `opacity` / `borderColor` / `borderWidth` / `borderDash`
  存在地图文件里，所以改设置不会悄悄改掉你已经画好的地图；
- 区域类型的不透明度与边框目前**只画在画布上**：SVG / PNG 导出与 Base 缩略图里区域仍是固定的半透明度、
  不画边框虚线（这是升级前就有的缺口）。

### 定义文件：把自定义定义带到别的库

你定义的**自定义地形、标记图标、路径类型、区域类型**都只存在**插件设置**里（每个库各一份）。
想把它们备份、带到别的库、或分享给别人，就用**定义文件** ——
右侧边栏「地图面板」的「文件与导出」一组里有两个动作，命令面板里也有同名命令（同一份实现）：

| 命令 | 作用 |
|---|---|
| **导出定义文件…（自定义地形/标记/路径类型/区域类型）** | 把四类**自定义**定义打包成一份 JSON |
| **导入定义文件…** | 从库里的 `.json` 文件中挑一份，**先看确认对话框，再决定导入** |

- **文件写在库根目录**，名字形如 `project-kaki-definitions-20260925-1130.json`；**重名绝不覆盖**，自动加 `-2`、`-3`；
- **内置类型不进文件**：内置 9 种地形 / 9 种图标 / 4 种路径类型 / 6 种区域类型在每个库里本来就有，
  写进去只会得到一串"已有同 ID"。代价是"内置类型的参数不随文件分享"—— 这是取舍，不是遗漏；
- **导入只做补充，绝不删东西**：同 ID 的条目**保留你现有的定义**（冲突会记进"跳过 N 条"并写明原因）；
  文件里**没有的那一段**（例如一份老文件里没有"区域类型"）**不会改动你现有的对应定义**；
- **导入前先算给你看**：确认对话框里写明"将新增 N 条（地形 x · 标记 y · 路径类型 z · 区域类型 w）"、
  跳过哪些及原因、哪一段文件里没有 —— 对话框说的和真的会做的来自**同一份计划**，不可能不一致；
  没有可新增的条目时确认按钮是灰的（正文已经解释过为什么一条都进不来）；
- **幂等**：刚导出的文件立刻再导入 = **0 新增**，设置逐字段不变；
- 格式版本**接受 1 与 2**；**比插件支持的版本更高时明确拒绝**并提示升级插件
  （"尽力解析"等于骗你说导入成功了）；空文件在解析阶段就被拒绝；
- 设置里一条自定义定义都没有时**干脆不写文件**（写一个空文件只会让你以为导出成功了）；
- 读取失败 / 解析失败都会给出一条**可读原因**，而且**不会改动任何设置**。

### 图层开关与图例

**六个图层开关就在右侧边栏的「地图面板」里**（设置 → Project Kaki → 「图层」里也有同一组，
两边改的是同一份设置）。各层管什么：

| 图层 | 管什么 |
|---|---|
| 地形 | 六边形地形的填色 / 图片 |
| 网格 | 六边形网格线 |
| 区域 | 半透明的领地范围 |
| 路径 | 河流 / 道路 / 贸易路线 / 边界 |
| 标记 | 地标标记**与文字标注** |
| 名称 | 路径与区域的**名称文字** |

- 图层只是**显示开关**，不会改动地图数据：隐藏路径后文件里的路径仍然在，重新打开就回来了。
  想看"只剩地形"的干净底图、或者只想检查河流走向时，比删掉再画回来安全得多。
- 图层状态存在**插件设置**里（唯一的真相），不写进地图文件 —— 换台机器打开同一张地图，
  地图内容一样，显示偏好各随各的设置。
- **停用地图层**的入口是「地图面板」里的动作，或命令面板（`Ctrl/Cmd+P`）里的同名命令。
  画布工具条上原来也有一个「地图层」按钮，但它会随地图层一起消失 —— 关掉之后就再也点不到它，
  所以已经移除：**一个能把自己弄没的按钮不适合承担这个职责**。
- 「显示图例」打开后，画布右下角出现一块图例：**内容来自地图上实际有的东西**（不是写死的清单），
  地形按格数、路径按类型（色块会体现虚线）、区域按颜色归并，并在右侧给出数量。
  隐藏某一层时，图例里对应的条目也会消失。
- 图例是"看"的东西，不抢画布手势（悬停、拖动、缩放照常）。

### 地图面板与开发者模式

左侧边栏的地图图标 → 在右侧边栏打开**「地图面板」**。面板顶部是**六个图层开关**（边看画布边切），
下面按用途分组列出常用动作（地图层 / 编辑 / 文件与导出）；点按钮就等于执行命令，
用不上的按钮显示为灰色，状态说明在**悬停提示**里。也可以在设置页点「打开面板」。

设置里的 **开发者模式**（默认关闭）控制两个探针命令（诊断当前 Canvas / 监视视口变化）：
关闭时它们从命令面板里**隐藏**（不是灰掉），避免误触。

## Base 视图：把地图和笔记连起来

给任意笔记写上坐标，它就会出现在地图的 Base 视图里：

```yaml
---
coordinates: [320, -140]   # 也接受 {x: 320, y: -140} 或 "320,-140"
map-type: city             # city/town/fortress/ruin/port/temple/mountain-peak/cave/tower
region: 北境王国            # 可选，用于分组
---
```

在 `.base` 里把视图类型选成 **地图**（id 为 `fictional-map`），你会得到：

- 一张**表格**：地图上的标记/路径/区域 + 库里带坐标的笔记，`来源` 列区分「笔记」/「地图」；
- 一个**地图缩略图**：地形色块、路径、区域、标记与笔记点，**都可以点击跳转**到对应文件；
- 坐标写错（例如 `coordinates: 东边那座城`）的行会被**标红**并在表格下方列出问题笔记，
  而不是静默忽略。

视图选项：地图文档 / 坐标属性 / 类型属性 / 地区属性 / 排序方式。
未配置地图文档时，库里只有一张地图会自动选用，多张时只显示笔记侧并给出提示。

## 导出

| 命令 | 产物 | 说明 |
|---|---|---|
| **导出地图…（可选范围与格式）** | 自选 | 一个对话框里同时选**范围**与**格式**（SVG / PNG） |
| **导出当前地图为 SVG** | `<地图名>.svg` | 快捷方式 =「全部内容 + SVG」，矢量图可无损缩放 |
| **导出当前地图为 PNG** | `<地图名>.png` | 快捷方式 =「全部内容 + PNG」，1600×1000 位图，方便直接贴到别处 |

三种**导出范围**：

| 范围 | 含义 |
|---|---|
| **全部内容**（默认） | 扫全部内容取包围盒 —— 与老版本的行为完全一致 |
| **当前视口** | 取画布**当前可见的世界矩形**：先把画布平移/缩放到想要的那一块，再导出 |
| **某个区域** | 从下拉里选一个你**画过的区域**，按它的范围（外加一点留白）导出 |

**为什么要能选范围**：导出（以及 Base 缩略图）是"把**全部内容**等比缩进画布"，
所以**一个远离主体的孤立格会把整张图压小**。实现时量过的一个例子：同一块 200×200 的区域，
按「某个区域」导出时它在图里占高约 **709/1000 px**，而按「全部内容」导出只剩约 **10/1000 px**
（具体比例取决于你地图上离群内容有多远）。
想清楚自己要哪一块、就选哪个范围，这种"被远处的点拖累"的情况就不会发生。

> 一条容易被误读的语义：**按范围导出不是"丢掉"范围外的内容**。
> 内容仍然全部绘制，只是落在该范围的视野之外、由 SVG 的视野自然裁掉 ——
> 换句话说，选范围只是换了个取景框，地图数据一个字节都没少。

其余行为：

- 产物写在**地图文件同目录**，重名自动加 `-2`、`-3`（不覆盖已有文件），导出后自动打开；
- **文件名带上范围**，方便一次会话里导好几张：`全部内容` 不加后缀（老名字不变）、
  `当前视口` → `Los-视口.png`、按区域 → `Los-北境领.png`
  （区域名里的 `/`、`:` 这类字符会被换成 `-`；没起过名字的区域用「未命名区域 N」）；
- 格式**同源**：PNG 就是先出 SVG 再光栅化，共用同一套世界坐标与配色，导出的图与画布一致；
- 导完的提示里会写明**这次用的是哪个范围**，方便确认导对了没有；
- **失败时对话框留在原地**并给出可读原因（例如"当前环境不支持把 SVG 转位图，请改用 SVG"），
  而且**只给一条提示、不产出空文件** —— 免得你以为导出成功了。

## 设置

设置 → 社区插件 → Project Kaki：

| 项 | 默认 | 说明 |
|---|---|---|
| 路径与区域名称的字号 | 1× | 名称随缩放变大但有不小于下限的实际字号；这里整体缩放，右侧直接显示换算后的 CSS 字号 |
| 显示六边形网格 | 开 | 关闭后地形照常绘制，只是不画网格线 |
| 路径类型 | 内置 4 种 | 内置 4 种 + 你自己的类型；每种可调颜色 / 线宽 / 虚线 / 端点 / 连接。内置 4 种的名字固定，参数可改 |
| 区域类型 | 内置 6 种 | 内置 6 种（王国 / 帝国 / 公国 / 教区 / 荒原 / 海域）+ 你自己的类型；每种可调填充色 / 不透明度 / 边框色 / 边框宽 / 边框虚线 |
| 自定义地形 | 无 | 每条 = ID + 显示名 + 颜色 + 可选字形/图片；ID 是写进文件的数据，显示名只是界面文案 |
| 自定义标记 | 无 | 每条 = ID + 显示名 + 字形或库内图片（两种模式）；ID 是写进文件的数据，显示名只是界面文案 |
| 图层开关（6 个） | 全显示 | 地形 / 网格 / 区域 / 路径 / 标记 / 名称；只影响显示，**不写进地图数据** |
| 图例 | 隐藏 | 从地图实际内容生成；显示在画布右下角，不抢手势 |
| 名称字体族 | 跟随主题 | 可写字体列表（如 `Noto Serif SC, serif`）；只接受字体族，整条 CSS font 简写会被拒绝 |
| 恢复出厂样式 | — | 一键把**内置**路径类型与区域类型的参数、字体还原（自定义类型的定义不会被删） |
| 开发者模式 | 关 | 打开后才出现两个诊断用探针命令 |

> **颜色只影响新画的对象。** 路径与区域把自己的颜色存在地图文件里（`path.color` / `region.color`），
> 所以改设置**不会**悄悄改掉你已经画好的地图 —— 想统一改色请重画，或直接编辑文件里的 `color` 字段。

> **类型定义（`custom:`）与显示名是两件事。** 写进地图文件的是 ID，显示名只是界面文案：
> 改显示名、甚至删掉一条定义，都不会改动你已经画好的对象（只是视觉回退）；
> 反过来，认不出的类型也**原样保留**，不会被改写成别的值或整条丢弃。

## 数据与文件

插件只创建三类文件，都是**纯文本、可 Git 管理、可手工编辑**：

- **地图文档** `Maps/<名字>.map.md`：frontmatter 记录绑定关系，正文里一段 fenced JSON 是地图数据；
- **Base 文件**（可选，由命令生成）：YAML；
- **定义文件**（可选，由命令导出）：一份 JSON，装着四类**自定义**定义，用来备份或分享（见上）。

```yaml
---
type: fictional-cartographer-map     # 地图文档的识别标记（这个值不会改名，见下）
fc-version: 1
name: "World"
canvases:
  - "Maps/World.canvas"
---
```

设计上的承诺：

- **未知字段原样保留**（前向兼容）；`fc-version` 高于插件支持的版本时**只读打开，绝不写回**；
- 地形键按键排序后序列化，**Git diff 稳定**；
- **安全闸**：写入前检查文件里有没有 `type: fictional-cartographer-map`，拒绝覆盖普通笔记；
- 单条目错误只跳过该条目并告警，其余数据保持可用。

> 关于几个"看起来像旧名字"的标识：`fictional-cartographer-map`（地图文档类型）、
> `fictional-map`（Base 视图 id）是**写在你的文件里**的持久化标识，改名会让已有地图与 Base 失效，
> 因此永远保持原样；插件的显示名与目录才是 `Project Kaki` / `project-kaki`。

## 兼容性与已知限制

- **Obsidian 1.5.0+**；**Base 自定义视图需要 1.10.0+**。版本不够时会给出明确提示并
  **降级为仅 Canvas 功能**，不会静默失败。
- 插件内部使用了 Obsidian Canvas 的**未公开 API**（官方没有提供地图/坐标接口）。
  已把它们全部隔离在 `src/canvas/CanvasAdapter.ts`，并对每个字段做**形状守卫**：
  探测失败就优雅停用并提示，而不是抛异常。版本升级后若出现异常，请运行
  **诊断当前 Canvas** 命令并把报告发出来 —— 那是排查版本漂移唯一需要的信息。
- **桌面向**：手势假设鼠标（双击、右键、中键），移动端尚未适配。
- Base 表格一次性建表，**未做虚拟滚动**（几百行会变慢）。
- 撤销栈上限 100 步，**不支持跨文件撤销**。
- **自定义地形的图片只画在画布上**：Base 缩略图与 SVG 导出用回退色（导出文件可脱离你的库打开，内联图片属于后续工作）。
- **标记图标同样只画在画布上**：SVG / PNG 导出与 Base 缩略图里的标记一律是一个固定的小圆点，
  自定义字形与图片都不会出现在导出文件里。
- **区域类型的不透明度与边框也只画在画布上**：导出与 Base 缩略图里区域仍是固定的半透明度、不画边框虚线。
- **定义文件只装自定义类型**：内置 9 种地形 / 9 种图标 / 4 种路径类型 / 6 种区域类型的参数不随文件分享。

## 常见问题

**网格和卡片对不齐 / 放大后发糊？**
网格与原生卡片处于同一世界坐标系。若对不齐，先运行 **诊断当前 Canvas**：它会报告挂载点判定、
矩阵缩放、坐标量化粒度与实测标定结果。

**名称太小 / 太大？**
设置里调「路径与区域名称的字号」。名称承诺的是 **CSS 像素**，且随画布缩放变化；
设置页会显示换算后的实际字号，便于直接读数。

**为什么地形画在卡片下面？**
绘制顺序 = 命中测试顺序。地形需要在卡片之下，覆盖层就必须插在宿主最前面，
因此它**永久不参与指针事件**；绘制手势在视图容器上以捕获阶段监听，并排除原生控件。

**在旧版 Obsidian 上会怎样？**
1.10.0 以下只注册 Canvas 功能并给出提示；1.5.0 以下不做任何事。不会报错。

## 开发

```bash
node scripts/build.mjs                            # 构建（自研：TypeScript 编译器 API + 模块内联 → main.js）
node node_modules/typescript/bin/tsc --noEmit     # 类型检查（0 错是底线）
node --test --test-isolation=none                 # 404 个单元测试
node scripts/smoke.mjs                            # 冒烟：加载真实 main.js + 假 Obsidian，37 个场景 / 982 条断言
node scripts/deploy.mjs                           # 部署到隔离测试库（默认 E:\ObsidianPulgins\test-vault）
```

**提交或发布前请按顺序跑完前四条命令。** 冒烟测试加载的是打包产物 `main.js`，
不重新构建就会拿旧产物跑测试。

```
src/          插件源码（core 纯函数 · canvas 私有 API 隔离层 · render 绘制 · editor 交互 · base Base 视图 · ui）
tests/        单元测试（node:test，纯函数优先）
scripts/      build / smoke / deploy / make-preview
docs/         用户手册 · 工程笔记 · 技术方案 · 各阶段实施记录
```

想深入的话，按这个顺序读：

1. [`docs/USER-MANUAL.md`](docs/USER-MANUAL.md) — 面向使用者的完整手册（含故障排查）；
2. [`docs/ENGINEERING-NOTES.md`](docs/ENGINEERING-NOTES.md) — **接手必读**：踩过的坑、测试策略、
   未验证项。其中 §5「已经踩过的坑」是这个项目最贵的东西；
3. [`docs/HANDOFF.md`](docs/HANDOFF.md) — 当前基线与下一步；
4. [`docs/TECHNICAL-DESIGN-v2.md`](docs/TECHNICAL-DESIGN-v2.md)、
   [`docs/PHASE-0-RESULTS.md`](docs/PHASE-0-RESULTS.md) — 设计推导与平台实测数据。

## 路线图

- **Phase 3 验收**：Base 缩略图点击跳转、极端容器比例下的等比例显示（已在真实库确认可见，交互待逐项复核）；
- **样式与图层**：✅ 已支持自定义路径/区域颜色、名称字体族、自定义地形、图层开关与图例；
  剩余：把图例画进导出文件、把样式设置一键应用到已有对象（需要一个可撤销的新操作）；
- **Phase 4**：✅ SVG 与 PNG 导出已完成；剩余：多图层导出（分层出图）；
- **Phase 5**：把自定义地形从"颜色 + 字形 + 图片"扩展到图块与变体；
- **Phase 6**：移动端与触控笔（长按代替右键、单指绘制、双指平移缩放）。

## 许可证

[Apache License 2.0](LICENSE)。

## 签名

这个插件是一个人跟一个 AI 结对做出来的，两边都签在这里。

**Shir0Tak1na** —— 设计者、出题人、验收人。提出「在 Canvas 里画虚构地图」这个想法，
做全部产品与设计决策，在真实库里一轮一轮试手感、报缺陷。这个项目每一个转折点都来自他的反馈：
提出给区域与线条命名、指出「名称没有办法顺着路自动适应」、指出「河流绘制过后直接变为了折线段」，
以及那句直接点破根因的「调整滑块，字体只有阴影在变化」——`ctx.font` 里的 `var()` 让整条字体声明
非法、被浏览器静默忽略，我自己推导了两轮都没找到。他还定义了这个项目的节奏：小步交付、
每一步都要能亲眼验证、错了就说错。

**DeepSeek Harness Agent** · `deepseek-v4-flash-vision-exp` —— 写代码、写测试、写文档。
账也记在文档里，不藏：三次"数值改对了但机制没生效"、两次把断言写在实现细节上
（所以"河流变折线"这种东西能全绿通过）、一次忘了触发异步加载（界面无报错但数据永远是 0）。
这个项目里可靠的部分都是**量出来**的，不是猜的 —— 坐标换算、量化误差、位图与 CSS 像素的比例、
字号的实际大小，全部以实测为准。

```
Shir0Tak1na                                          2026-09-23
DeepSeek Harness Agent · deepseek-v4-flash-vision-exp  2026-09-23
```
