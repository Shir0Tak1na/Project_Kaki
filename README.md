...
改了会让已有地图、已保存的 Base 与侧栏布局失效（理由见 §5.6）。

> **给接手者的说明**：这份 README 是**交接文档**，不是宣传页。它包含当前真实进度、
> 每个文件干什么、本环境下的构建/验证命令、**已经踩过的坑（务必先读 §5，否则会重复我犯过的错）**，
> 以及明确的待办与未验证项。深水区的推导过程在 `docs/` 下，正文会指路。
